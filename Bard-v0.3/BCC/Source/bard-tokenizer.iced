...
TokenType =
{
  definitions: [
    ...
    { token_type:"EOL", structural:true }
    { token_type:"COMMENT", structural:true }

    { token_type:"DEFINITION", structural:true }
    { token_type:"LOCAL_DEFINITION", structural:true }
    { token_type:"ALIAS_ARG_INDEX", structural:true }

    { token_type:"ILLEGAL_KEYWORD", structural:true }

    # NON-STATEMENT KEYWORDS
    { keyword: "alias", structural:true }
    { keyword: "aspect", structural:true }
    { keyword: "augment", structural:true }
    { keyword: "catch", structural:true }
    { keyword: "CATEGORIES", structural:true }
    { keyword: "class", structural:true }
    { keyword: "case", structural:true }
    { keyword: "compound", structural:true }
    { keyword: "else", structural:true }
    { keyword: "elseIf", structural:true }
    { keyword: "endAspect", structural:true }
    { keyword: "endAugment", structural:true }
    { keyword: "endClass", structural:true }
    { keyword: "endCompound", structural:true }
    { keyword: "endContingent", structural:true }
    { keyword: "endDelegate", structural:true }
    { keyword: "endEnumeration", structural:true }
    { keyword: "endForEach", structural:true }
    { keyword: "endFunction", structural:true }
    { keyword: "endIf", structural:true }
    { keyword: "endLoop", structural:true }
    { keyword: "endPrimitive", structural:true }
    { keyword: "endWhich", structural:true }
    { keyword: "endWhichIs", structural:true }
    { keyword: "endTry", structural:true }
    { keyword: "endWhile", structural:true }
    { keyword: "ENUMERATE", structural:true }
    { keyword: "enumeration", structural:true }
    { keyword: "in", structural:true }
    { keyword: "METHODS", structural:true }
    { keyword: "library", structural:true }
    { keyword: "method", structural:true }
    { keyword: "of", structural:true }
    { keyword: "others", structural:true }
    { keyword: "primitive", structural:true }
    { keyword: "PROPERTIES", structural:true }
    { keyword: "routine", structural:true }
    { keyword: "satisfied", structural:true }
    { keyword: "SHARED", structural:true }
    { keyword: "task", structural:true }
    { keyword: "unsatisfied", structural:true }
    { keyword: "withTimeout", structural:true }

    # NON-STATEMENT SYMBOLS
    { SYMBOL_CLOSE_BRACE:      '}' , structural:true }
    { SYMBOL_CLOSE_BRACKET:    ']' , structural:true }
    { SYMBOL_CLOSE_COMMENT:    "}#", structural:true }
    { SYMBOL_CLOSE_PAREN:      ')' , structural:true }
    { SYMBOL_CLOSE_SPECIALIZE: ">>", structural:true }

    { SYMBOL_COMMA:     ',',  structural:true }
    { SYMBOL_FAT_ARROW: "=>", structural:true }
...
Tokenizer =
  ...
  convert_characters_to_token: (t, terminator, text) ->
    if (text.length == 1 and terminator == '\'')
      t.type = TokenType.LITERAL_CHARACTER
      t.value = text
    else
  ...
  define_tokens: () ->
    if (TokenType.definitions)
      ...
      TokenType.is_structural = {}
      TokenType.keyword_values = {}
      for i of TokenType.definitions
        def = TokenType.definitions[i]
        if (def.structural) then TokenType.is_structural[i] = true

        if (def.token_type)
          value = def.token_type
          ...
          if (def.value)
            TokenType.keyword_values[i] = def.value

        else if (def.illegal_keyword)
          value = def.keyword
    ...
    if (ch.is_letter() or ch == '_')
      ...
      if (type?)
        if (type != TokenType.ILLEGAL_KEYWORD)
          t = @create_token( type )
          value = TokenType.keyword_values[type]
          if (value) then t.value = value
          return t
        else
  ...
  scan_character: (t) ->
    if (not @scanner.has_another())
      throw t.error( "Unterminated literal string." )
  ...
  tokenize_characters: (terminator) ->
    t = @create_token( TokenType.LITERAL_STRING )
    ...
    while (@scanner.has_another())
      ...
      if (ch == terminator)
        @scanner.read()
        return @convert_characters_to_token( t, terminator, buffer )
      else
        buffer += @scan_character(t)
