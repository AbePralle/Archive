...
ES2Renderer* plasmacore_renderer;
...
{    
  if ((self = [super initWithFrame:bounds]))
  {
    ...
    plasmacore_renderer = [[ES2Renderer alloc] init];
  ...
  plasmacore_view_controller = [[[PlasmacoreViewController alloc] init] retain];
...
- (void)drawView:(id)sender
{
  ...
  catch (int error_code)
  {
    ...
    if (bard_error_message.value) [plasmacore_app_delegate alert:bard_error_message.value];
  }
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
  for (UITouch* touch in touches)
  {
    ...
    if (index > 0)
    {
      plasmacore_queue_signal( plasmacore.event_mouse_button,
        index, 1, true, x, y );
    }
  }
}
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
  for (UITouch* touch in touches)
  {
    ...
    if (index > 0)
    {
      plasmacore_queue_signal( plasmacore.event_mouse_move, 
        index, 0, true, x, y  );
    }
  }
}
...
- (BOOL) textField:(UITextField*)field shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString*)string
{
  ...
  plasmacore_queue_signal( plasmacore.event_key, 1, ch, true );
  plasmacore_queue_signal( plasmacore.event_key, 1, ch, false );
  field.text = @"     ";
...
{
  BardReal64 x =  acceleration.x;
  BardReal64 y = -acceleration.y;
  BardReal64 z =  acceleration.z;
