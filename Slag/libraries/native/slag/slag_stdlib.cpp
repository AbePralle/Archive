...
void Math__abs__Int64()
{
  ...
  if (*addr < 0) *addr = -(*addr);
  SLAG_DISCARD_REF();  // Discard context object
}
...
{
  SLAG_POP_REF();
  SLAG_PUSH_LOGICAL( true );  // TODO
}

void Network__is_mobile()
{
  SLAG_POP_REF();
  SLAG_PUSH_LOGICAL( false );  // TODO
}

void Network__mac_address()
{
  SLAG_POP_REF();
  SLAG_PUSH_REF( SlagString::create("(Network.mac_address not implemented)") );  // TODO
}
#endif
...
#else
  SLAG_PUSH_INT32( (SlagInt32)(int)context );
#endif
}
...
void SHA1__hmac__String_String()
{
  ...
  SLAG_POP_REF();
  SLAG_POP_REF();
  SLAG_PUSH_REF( SlagString::create("0000000000000000000000000000000000000000") );
}
#endif // !ANDROID
...
{
  SLAG_DISCARD_REF();  // singleton ptr

  if ( ! *slag_language_ref )
  {
    slag_language_ref = (SlagObject*) SlagString::create((char*)"en");
  }
